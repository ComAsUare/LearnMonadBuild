...
7. 合约开发链路
    1. postgre关系型数据库
        1. PG 作为关系型数据库，最擅长建立表与表之间的外键关联。它能把链上的地址和Web2 的数据库表（如用户表、社交表）完美地 JOIN 结在在一起
        2.Web3 索引领域（例如著名的 The Graph 协议底层、各种链游后端、NFT 交易所后端），PostgreSQL 几乎是默认首选，这得益于它的几个硬核特性：

            完美的 JSONB 支持： 区块链上的智能合约经常会抛出格式不固定的日志（Logs）或元数据（Metadata）。PG 的 JSONB 类型不仅能存这些半结构化数据，还能直接对 JSON 内部的字段建索引，兼顾了 NoSQL 的灵活性和关系型数据库的严谨。

            对大数（BigInt）的友好支持： 以太坊上的代币精度通常是 18 位小数，数据极大（以 Wei 为单位）。PG 原生支持超大整数和高精度浮点数，不会像某些数据库那样发生精度截断。

            强大的插件生态（如 TimescaleDB）： 链上数据天然带有很强的“时间戳”属性（每个区块都有时间）。通过给 PG 挂载 TimescaleDB 插件，它能瞬间变成一个时序数据库，处理链上历史K线图、交易流水图极其丝滑。
    2. 去中心化存储
        旨在打破中心化存储垄断
        IPFS（InterPlanetary File System）
            内容寻址（Content Addressing）, 不保证永久
        Arweave
            织巢网络（Blockweave），绝对永久
    3. dapp开发流程
        1. 需求分析与规划
            功能需求（转账、查询余额、创建投票），区块链平台（eth,solana,polygon） ，ux(用户界面及交互流程)
        2. 编写合约，测试审计优化
        3. indexer捕获链上事件数据，存入数据库表内
            弄清前端数据需求；ponder 和 subgraph 事件数据清理和存入；部署运维，docker
