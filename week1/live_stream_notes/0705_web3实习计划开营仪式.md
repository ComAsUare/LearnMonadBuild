...
2. marcus分享monad：
    ...
    5. 除了Parallel Execution， monad在基础层使用的原语：asynchronous excution流水线化共识和执行，以及monadDB状态查询
        * 1. asynchronous excution
            传统共识-执行交错，共识耗时>>执行。
        * 2. optimistic parallel excution
         保持串行顺序的结果，过程并行计算。
        * 3. monadDB
        应用层和内核共享无锁环形队列传递任务。 
    6. monad在共识和runtime中使用的技术：
        ...
        * 2. monadBFT 共识
        * 3. raptocast 区块传播
    7. 开发文档https://docs.monad.xyz/，for ai:https://docs.monad.xyz/llms-full.txt
    skills.monad.com: 内置水龙头，合约前端开发部署，索引器部署
    8. 开发工具monadfoudry, 和viem
    9. 其他资源： ai blueprint, delta v构建创始人社区
